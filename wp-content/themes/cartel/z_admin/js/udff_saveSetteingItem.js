...
jQuery(document).ready(function(){	
	jQuery('input[id^="settingItem_"]').click(function(){
		...
		var settingItemButtonType = (jQuery(this).prop('value') == 'Save');
		var id_string = jQuery(this).parent().find('input:first').prop('id');
		...
		if(jQuery(this).hasClass('thisIsTextarea')) {
			...
			settingValue_string = (settingItemButtonType) ? jQuery(this).parent().find('select:first').val() : jQuery(this).parent().find('.select_initValue').prop('value');
			...
			if(settingItemButtonType) {
				...
				settingValue_string =jQuery(this).parent().find('.select_initValue').prop('value');
			}
			...
			settingValue_string = (settingItemButtonType) ? jQuery(this).parent().find('input:first').prop('value') : jQuery(this).parent().find('input:first').prop('defaultValue');
	...
	function sendDataToServer (that, id_string, settingValue_string) {
		...
		function(responseData)
		{
			...
			if (responseData.result == 'ok') {
				...
				jQuery(that).parent().find('input[id^="settingItem_save_"]').removeClass('saveProgress').addClass('saveSuccess').prop('value','OK');
				...
				jQuery(that).parent().find('input[id^="settingItem_save_"]').addClass('saveError').removeClass('saveProgress').prop('value','ERROR');
				setTimeout(function(){
            		SaveReset(that);
            	},2000);
			}
	...
	function prepareForSaving (that) {
		jQuery(that).parent().find('input[id^="settingItem_"]').removeClass('saveDisabled').removeClass('saveSuccess').removeClass('saveError').removeClass('saveProgress').prop('value','');
...
jQuery(that).parent().find('input[id^="settingItem_save_"]').addClass('saveDisabled').removeClass('saveSuccess').removeClass('saveError').removeClass('saveProgress').prop('value','Save');
		jQuery(that).parent().find('input[id^="settingItem_revert_"]').addClass('revertDisabled').prop('value','Revert').fadeIn();
