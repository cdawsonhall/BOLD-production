...
(function($){
	...
	wpLink = {
		...
		init : function() {
			...
			if($('.link-class-field-wrapper').length==0) {
				$('.link-target').before('<div class="link-class-field-wrapper"><label><span>Class</span><input id="link-class-field" type="text" name="linkclass" /></label></div>');
			}
...
//			$('#internal-toggle').click( wpLink.toggleInternalLinking );
		...
		mceRefresh : function() {
			...
			if ( e = ed.dom.getParent(ed.selection.getNode(), 'A') ) {
				...
				inputs.class.val( ed.dom.getAttrib(e, 'class') );
