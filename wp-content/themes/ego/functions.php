...
function load_udt_scripts() {
	...
	wp_enqueue_script('fancybox', get_template_directory_uri() . '/js/jquery.fancybox-1.3.4.js', array('jquery','jquery-ui'), false, true);
	wp_enqueue_script('metadata', get_template_directory_uri() . '/js/jquery.metadata.js', array('jquery'), false, true);

	wp_enqueue_script('masonry', get_template_directory_uri() . '/js/masonry.pkgd.js', array(), false, true);
...
// The old tweets shortcode. No longer in use after Twitter's API update.
/*
...
function sc_round_thumb( $atts, $content = null ) {
	...
	} else if($lightbox=='soundcloud') {
		$lightbox_class='lightbox-soundcloud';
	}
	...
	if($link!='' && $lightbox=='soundcloud') {
		$link='https://w.soundcloud.com/player/?url='.$link.'&amp;auto_play=true&amp;show_artwork=true';
	}
...
function sc_lightbox( $atts, $content = null ) {
	...
	if($media_type=='image') {
		...
		$output='<div class="thumb '.$align.''.$no_overlay.'" style="width:'.$width.'; height:'.$height.'; '.$style.'"><a class="lightbox-soundcloud" href="https://w.soundcloud.com/player/?url='.$url.'&amp;auto_play=true&amp;show_artwork=true" title="'.esc_attr($link_title).'"><img src="'.esc_url($thumbnail_url).'" style="width:'.$width.'; height:'.$height.';" alt="" /></a></div>';
		if($content!='') { $output.='<div class="fancybox-html">'.wptexturize(wpautop($content)).'</div>'; }
	}
	return $output;
}
